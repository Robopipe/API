...
from robopipe_api.models.dashboard.eval_models import EvalLimitItemEdge, EvalLimitItemParameter
...
class TestPositionalEvaluation:
    def test_pos_left_all_in_range(self):
        # Detections at center x=0.4 and x=0.6 → distance from left = 40% and 60%
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=30, limit_to=70,
    ...
    def test_pos_left_one_outside_range(self):
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=30, limit_to=50,
                               parent_edge=EvalLimitItemEdge.LEFT)
        evaluator = LimitItemEvaluator(item)
        targets = [
    ...
    def test_positional_no_targets_returns_false(self):
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=0, limit_to=100,
                               parent_edge=EvalLimitItemEdge.LEFT)
        evaluator = LimitItemEvaluator(item)
    ...
    def test_positional_with_parent_reference(self):
        ...
        # Detection center: (0.5, 0.5) → |0.5-0.2|/0.6*100 = 50%
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=40, limit_to=60,
                               parent_edge=EvalLimitItemEdge.LEFT)
        evaluator = LimitItemEvaluator(item)
    ...
    def test_positional_not_in_any_parent(self):
        ...
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=0, limit_to=10,
                               parent_edge=EvalLimitItemEdge.LEFT)
        evaluator = LimitItemEvaluator(item)
    ...
    def test_pos_center(self):
        ...
        item = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=0, limit_to=10,
                               target_edge=EvalLimitItemEdge.CENTER, parent_edge=EvalLimitItemEdge.CENTER)
        evaluator = LimitItemEvaluator(item)
        targets = [make_detection(coords=(0.4, 0.4, 0.6, 0.6))]
        result, _, _, _ = evaluator.evaluate(targets, targets, None)
        assert result is True
    ...
    def test_pos_top_and_bottom(self):
        # center y=0.3 → from top=30%, from bottom=70%
        item_top = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=25, limit_to=35,
                                   parent_edge=EvalLimitItemEdge.TOP)
        item_bot = make_limit_item(EvalLimitItemParameter.POSITION, limit_from=65, limit_to=75,
                                   parent_edge=EvalLimitItemEdge.BOTTOM)
