...
class ThresholdTracker:
    ...
    def record(self, config_id: int, test_case_id: str, passed: bool) -> None:
        ...
        tc_state = config_state.setdefault(
            test_case_id, {"total": 0, "failures": 0}
        )
        tc_state["total"] += 1
        if not passed:
            tc_state["failures"] += 1
    ...
    def get_status(
        ...
        for tc in test_cases:
            ...
            zone_name, zone_color = self._determine_zone(tc, pass_rate)
    ...
    def _determine_zone(
        ...
        for threshold in sorted_thresholds:
            if pass_rate < threshold.value:
                return threshold.name, threshold.color
