...
class DS2438(MySensor):  # vdd + vad + thermometer
    ...
    def full(self):
        self.value = (
            getattr(self, "VDD", None),
            getattr(self, "VAD", None),
...
class DS2408(MySensor):
    def __init__(
        ...
        self.major_group = major_group
        self.interval = bus.interval if interval is None else interval  # seconds
        self.is_dynamic_interval = (
            is_dynamic_interval  # dynamically change interval #TODO
        )
        self.last_value = None
        self.value = None
        self.lost = False
...
def MySensorFabric(
    ...
    if (sensor_type == "DS18B20") or (sensor_type == "DS18S20"):
        ...
        (sensor_type == "DS2408")
        or (sensor_type == "DS2406")
        or (sensor_type == "DS2404")
        or (sensor_type == "DS2413")
    ):
        return DS2408(
            address,
            sensor_type,
...
class OwBusDriver:
    ...
    async def set(
        self, scan_interval=None, do_scan=False, interval=None, do_reset=None
    ):
        was_changed = False
    ...
    async def scanning(self, server):
        while True:
            ...
            with anyio.CancelScope() as scope:
                ...
                await anyio.sleep(
                    self.scan_interval if self.scan_interval > 0 else 3600
                )
        ...
        async with ow.events as events:
            async for msg in events:
                ...
                if isinstance(msg, event.DeviceLocated):
                    ...
                    try:
                        mysensor = next(
                            x for x in self.mysensors if x.address == address
                        )
                        logger.info("Sensor found " + str(mysensor.circuit))
                    except Exception:
                        mysensor = MySensorFabric(
                            address, sensor_type, self, self.interval
                        )
                    if mysensor:
